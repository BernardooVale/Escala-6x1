...
def obter_dados(conexao):
    ...
    SELECT p.nome,
           ...
           AVG(pc.pib_per_capita) AS media_pib_per_capita
    FROM pais p
...
def criar_grafico(dados, arquivo_imagem='grafico.png'):
    nomes, medias_horas, medias_pib = zip(*dados)
    
    # Criar gráfico de dispersão
    plt.figure(figsize=(10, 6))
    plt.scatter(medias_pib, medias_horas, color='blue', label='Pontos (País)', zorder=3)
    
    # Adicionar rótulos aos pontos
    for i, nome in enumerate(nomes):
        plt.text(medias_pib[i], medias_horas[i], nome, fontsize=9, ha='right')

    # Ajustar curva de tendência (polinômio de grau 2 - curva quadrática)
    coef = np.polyfit(medias_pib, medias_horas, 2)  # Ajuste quadrático
    tendencia = np.poly1d(coef)
    x_vals = np.linspace(min(medias_pib), max(medias_pib), 100)  # Gerar valores x para a linha curva
    plt.plot(x_vals, tendencia(x_vals), color='red', linestyle='-', label='Curva de Tendência', zorder=2)

    # Configurar o gráfico
    plt.title('Relação entre PIB per Capita e Horas Trabalhadas por País')
    plt.xlabel('Média de PIB per Capita')
...
if dados:
    ...
    arquivo_imagem = 'grafico_pib_per_capita_horas_por_pais.png'
